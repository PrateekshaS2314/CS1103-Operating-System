...
echo -e "use 'kill -STOP $task_pid' command to pause the background manually"
...
echo -e "\nResuming the background task..."
kill -CONT $task_pid
#Resuming code manually
echo -e "use the command 'kill -CONT $task_pid' to resume the background manually"
echo -e "use the command 'ps -o pid,stat,cmd -p $task_pid' to check the status"
read -p "press enter to continue..."
